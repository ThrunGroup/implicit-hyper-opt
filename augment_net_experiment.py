...
def experiment(args, device):
    ...
    def test(loader, device, do_test_augment=True, num_augment=5):
    ...
    def hyper_step(elementary_lr, do_true_inverse=False):
        ...
        for batch_idx, (x, y) in enumerate(val_loader):
            val_loss = val_loss_func(x, y, device)
            optimizer.zero_grad()
    ...
    init_time = time.time()
    val_loss, val_acc = test(val_loader, device)
    ...
    for epoch in range(0, args.num_finetune_epochs):
        ...
        if not args.only_print_final_vals:
            ...
            val_loss, val_acc = test(val_loader, device, do_test_augment=False)
